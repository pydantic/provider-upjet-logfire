...
import (
	...
	apisCluster "github.com/pydantic/provider-upjet-logfire/apis/cluster"
	apisNamespaced "github.com/pydantic/provider-upjet-logfire/apis/namespaced"
	"github.com/pydantic/provider-upjet-logfire/config"
	"github.com/pydantic/provider-upjet-logfire/internal/clients"
	controllerCluster "github.com/pydantic/provider-upjet-logfire/internal/controller/cluster"
	controllerNamespaced "github.com/pydantic/provider-upjet-logfire/internal/controller/namespaced"
	"github.com/pydantic/provider-upjet-logfire/internal/features"
	"github.com/pydantic/provider-upjet-logfire/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Logfire").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-logfire"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-logfire",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Logfire APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Logfire APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-logfire:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Logfire controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Logfire controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Logfire controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Logfire controllers")
	}
